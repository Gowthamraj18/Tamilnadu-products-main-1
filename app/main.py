...
async def _validate_order_items_stock(session: AsyncSession, products_payload: List[Dict[str, Any]]) -> Optional[JSONResponse]:
    ...
    for p in products_payload:
        ...
        if not prod:
            return _json_error(400, message="One or more products are no longer available")
        if prod.stock < qty:
            return _json_error(
                400,
                message=f'Not enough stock for "{prod.name}". Only {prod.stock} left.',
            )
    return None


async def _decrement_stock_for_order_items(session: AsyncSession, items: List[Dict[str, Any]]) -> Optional[JSONResponse]:
    """Reduce inventory when payment succeeds (same quantities as order line items)."""
    for item in items:
        pid = safe_int_id(item.get("product"))
        ...
        prod = result.scalar_one_or_none()
        if not prod:
            return _json_error(400, error="Order contains a product that no longer exists")
        if prod.stock < qty:
            return _json_error(
                400,
                error=f'Not enough stock for "{prod.name}". Only {prod.stock} left.',
            )
    for item in items:
        pid = safe_int_id(item.get("product"))
        qty = int(item.get("quantity") or 0)
...
async def admin_upload_image(request: Request, session: SessionDep, image: UploadFile = File(...)) -> JSONResponse:
    ...
    if not ext:
        return _json_error(400, message="Only JPG, PNG, or WEBP images are allowed")

    data = await image.read()
    max_size = int(settings.get("max_file_size") or (5 * 1024 * 1024))
    if not data:
        return _json_error(400, message="Image file is empty")
    if len(data) > max_size:
        return _json_error(400, message=f"Image too large. Max size is {max_size // (1024 * 1024)}MB")

    filename = f"product-{secrets.token_hex(12)}{ext}"
    file_path = Path(UPLOAD_DIR) / filename
    file_path.write_bytes(data)
    return JSONResponse(
        status_code=201,
        content={"success": True, "data": {"url": f"/uploads/{filename}"}, "message": "Image uploaded successfully"},
    )
...
async def admin_create_product(
    ...
    if payload.price < 0:
        return _json_error(400, message="Price cannot be negative")
    if payload.stock < 0:
        return _json_error(400, message="Stock cannot be negative")
    if payload.originalPrice is not None and payload.originalPrice < 0:
        return _json_error(400, message="Original price cannot be negative")

    now = datetime.now(timezone.utc)
    slug = await _unique_product_slug(session, payload.name)
    image = payload.image.strip() if payload.image else "/images/placeholder-product.jpg"

    p = Product(
        name=payload.name.strip(),
        slug=slug,
        description=payload.description.strip(),
        short_description=(payload.description.strip()[:140] if payload.description else None),
        category=payload.category.strip(),
        price=payload.price,
        original_price=payload.originalPrice,
        images_json=_json_dumps([image]),
        stock=payload.stock,
    ...
    return JSONResponse(
        status_code=201,
        content={"success": True, "data": serialize_document(product_orm_to_api_dict(p))},
    )
...
async def admin_update_product(
    ...
    if payload.name is not None:
        name = payload.name.strip()
        if not name:
            return _json_error(400, message="Name cannot be empty")
        p.name = name
        p.slug = await _unique_product_slug(session, name, exclude_product_id=p.id)
    if payload.description is not None:
        p.description = payload.description.strip()
        p.short_description = p.description[:140] if p.description else None
    if payload.category is not None:
        category = payload.category.strip()
        if not category:
            return _json_error(400, message="Category cannot be empty")
        p.category = category
    ...
    if payload.image is not None:
        ...
        p.images_json = _json_dumps([image] if image else [])
...
if _FRONTEND_DIST.is_dir():
    ...
    async def _vite_icon() -> FileResponse:
        if _logo_png.is_file():
            return FileResponse(_logo_png, media_type="image/png")
        raise HTTPException(status_code=404)
...
if __name__ == "__main__":
    from .frontend_build import build_frontend_if_needed
