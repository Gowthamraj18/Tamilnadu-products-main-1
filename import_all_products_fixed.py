#!/usr/bin/env python3
"""
Script to import ALL products from frontend data into backend database
"""
...
def load_all_frontend_products():
    """Load ALL products from frontend data file"""
...
def convert_frontend_to_db(frontend_product):
    ...
    stock_data = frontend_product.get('stock', {})
    stock_keys = list(stock_data.keys()) if stock_data else []
    stock_value = stock_data.get(stock_keys[0], 0) if stock_keys else 0
    ...
    return {
        ...
        'stock': stock_value,
        'size_stock_json': json.dumps(stock_data),
...
async def import_all_products():
    ...
    frontend_products = load_all_frontend_products()
